<?php
if ( !class_exists('WPLPuc_v4p8_DebugBar_Extension', false) ):

	class WPLPuc_v4p8_DebugBar_Extension {
		const RESPONSE_BODY_LENGTH_LIMIT = 4000;

		/** @var WPLPuc_v4p8_UpdateChecker */
		protected $updateChecker;
		protected $panelClass = 'WPLPuc_v4p8_DebugBar_Panel';
		...
		private function getLibraryUrl($filePath) {
			...
			$absolutePath = WPLPuc_v4p8_Factory::normalizePath($absolutePath);

			$pluginDir = WPLPuc_v4p8_Factory::normalizePath(WP_PLUGIN_DIR);
			$muPluginDir = WPLPuc_v4p8_Factory::normalizePath(WPMU_PLUGIN_DIR);
			$themeDir = WPLPuc_v4p8_Factory::normalizePath(get_theme_root());
