...
if ( !class_exists('WPLPuc_v4p8_StateStore', false) ):

	class WPLPuc_v4p8_StateStore {
		/**
		...
		/**
		 * @var WPLPuc_v4p8_Update|null Cached update.
		 */
		...
		/**
		 * @return null|WPLPuc_v4p8_Update
		 */
		 ...
		 * @return $this
		 */
		public function setUpdate(WPLPuc_v4p8_Update $update = null) {
			$this->lazyLoad();
			$this->update = $update;
			return $this;
		}
		...
		public function save() {
			...
			if ( isset($this->update)) {
				...
				if ( WPLPuc_v4p8_Utils::startsWith($updateClass, $prefix) ) {
		...
		protected function load() {
			...
			$this->lastCheck = intval(WPLPuc_v4p8_Utils::get($state, 'lastCheck', 0));
			$this->checkedVersion = WPLPuc_v4p8_Utils::get($state, 'checkedVersion', '');
			$this->update = null;
