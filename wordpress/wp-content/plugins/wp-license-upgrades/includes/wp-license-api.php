...
add_filter('set-screen-option', 'wpl_table_set_option', 10, 3);

function wpl_table_set_option( $status, $option, $value ) {
	return $value;
}

function wpl_add_menu_activation(){
	add_menu_page(
		'WPLicense',
		'WPLicense',
		'manage_options',
		'admin.php?page=wpl_activation',
		'',
		plugins_url( 'libraries/images/wpl-icon.png', __FILE__ ),
		6
	);
	add_submenu_page( 'admin.php?page=wpl_activation', 'WPLicense Activation', 'Activation', 'manage_options', 'admin.php?page=wpl_activation' );
}

add_action( 'admin_menu', 'wpl_add_menu_activation' );

if ( ! class_exists( 'WPL_Plugin_Upgrades' ) ) {
	class WPL_Plugin_Upgrades {
		...
		public $ame_activation_username;
		...
		public function __construct( $file, $software_title, $software_version, $plugin_or_theme, $api_url, $text_domain, $extra ) {
			...
			if ( is_admin() ) {
				...
				add_action( 'admin_menu', array( $this, 'register_menu' ), 10000 );
				...
				$this->ame_activation_username     = 'activation_username';
				...
				$this->ame_username_key           = $this->data_prefix . '_username';
				...
				$this->ame_settings_menu_title         = $this->software_title . __( '', $this->text_domain );
				...
				if ( ! defined( 'WPL_ACTIVATION_EMAIL' ) ) {
					define( 'WPL_ACTIVATION_EMAIL', $this->ame_options[ $this->ame_activation_email ] );
				}
				if ( ! defined( 'WPL_GET_STATUS' ) ) {
					define( 'WPL_GET_STATUS', get_option( $this->ame_activated_key ) );
				}
				if ( ! defined( 'WPL_ACTIVATED' ) && defined( 'WPL_GET_STATUS' ) && WPL_GET_STATUS == 'Activated' ) {
					define( 'WPL_ACTIVATED', 'Activated' );
				}

				if ( ! empty( $this->ame_activated_key ) && get_option( $this->ame_activated_key ) != 'Activated' ) {
					add_action( 'in_admin_header', array( $this, 'inactive_notice' ), 100000 );
				} else {
					add_filter( 'wplicense_plugin_action_links', 'wpl_filter_action_links', 10, 4 );
				}
				$WPL_request_api = WPL_get( 'transient', 'wpl_request_api' );
				if ( !empty($WPL_request_api) && $pagenow == 'admin.php' && $_GET['page'] == 'wpl_activation' ) {
					wpl_script_request('wpl_request_api', 'wpl_request_api', 'spinner_request_api', false, false, false);
				}
			}

			if ( $this->plugin_or_theme == 'plugin' ) {
				if ( WPL_settings( 'deactivation' ) ) {
		...
		public function register_menus() {
			add_options_page( __( $this->ame_settings_menu_title, $this->text_domain ), __( $this->ame_settings_menu_title, $this->text_domain ), 'manage_options', 'wpl_activation', array(
				$this,
				'config_page'
			) );
		}
		...
		public function inactive_notice() { ?>
            ...
            <div class="notice notice-error">
                <p><?php printf( __( 'The <strong>%s</strong> API Key has not been activated, so the %s is inactive! %s<strong>Click here</strong>%s to activate <strong>%s</strong>.', $this->text_domain ), esc_attr( $this->software_title ), esc_attr( $this->plugin_or_theme ), '<a href="' . esc_url( admin_url( 'admin.php?page=wpl_activation&tab=' . $this->ame_activation_tab_key ) ) . '">', '</a>', esc_attr( $this->software_title ) ); ?></p>
            </div>
			<?php
		}
		...
		public function config_page() {
            ...
            <div class='wrap'>
				...
				<?php if (get_option( $this->ame_activated_key ) != 'Activated'){ ?>
					<hr>
					<div style="background:#ECECEC;border:1px solid #CCC;padding:0 10px;margin-top:5px;border-radius:5px;-moz-border-radius:5px;-webkit-border-radius:5px;">
						<p><strong>API Key:</strong> 41a96d6d9d9e58b39864b4f57da29d62545f2a9b<br>
						<strong>API Email: </strong><?php echo WPL_EMAIL ?><br>
						<span style="color: #ff0000;">Click the <strong>Save Changes</strong> button!</span><br></p>
					</div>
					<hr>
				<?php } elseif (get_option( $this->ame_username_key ) == null && !empty($WPL_subscription)){ ?>
					<hr>
					<div style="background:#ECECEC;border:1px solid #CCC;padding:0 10px;margin-top:5px;border-radius:5px;-moz-border-radius:5px;-webkit-border-radius:5px;">
						<p><strong><span style="color: #ff0000;">How to find your username? <a href="https://wplicense.com/my-account/username/" target="_blank">Click Here.</span></a></strong><br></p>
					</div>
					<hr>
				<?php } ?>
                ...
                <h2 class="nav-tab-wrapper">
					...
					foreach ( $settings_tabs as $tab_page => $tab_name ) {
						...
						echo '<a class="nav-tab ' . $active_tab . '" href="?page=' . 'wpl_activation' . '&tab=' . $tab_page . '">' . $tab_name . '</a>';
					}
					?>
                </h2>
		...
		public function load_settings() {
			...
			add_settings_field( $this->ame_api_key, __( 'API Key <span style="color: #ff0000;">*</span>', $this->text_domain ), array(
				$this,
				'wc_am_api_key_field'
			), $this->ame_activation_tab_key, $this->ame_api_key );
			add_settings_field( $this->ame_activation_email, __( 'API Email <span style="color: #ff0000;">*</span>', $this->text_domain ), array(
				$this,
				'wc_am_api_email_field'
			), $this->ame_activation_tab_key, $this->ame_api_key );

			$WPL_subscription = WPL_get( 'transient', 'wpl_subscription' );
			$required = !empty( $WPL_subscription ) ? 'WPLicense.com Username <span style="color: #ff0000;">*</span>' : 'WPLicense.com Username';
			add_settings_field( $this->ame_activation_username, __( $required, $this->text_domain ), array(
				$this,
				'wc_am_api_username_field'
			), $this->ame_activation_tab_key, $this->ame_api_key );
		...
		public function wc_am_api_key_field() {
			echo "<input id='api_key' name='" . $this->ame_data_key . "[" . $this->ame_api_key . "]' size='50' type='password' style='min-height: 30px;' value='41a96d6d9d9e58b39864b4f57da29d62545f2a9b' required readonly/>";
		...
		public function wc_am_api_email_field() {
			echo "<input id='activation_email' name='" . $this->ame_data_key . "[" . $this->ame_activation_email . "]' size='50' type='email' style='min-height: 30px;' value='wplicense@gmail.com' required readonly/>";
		...
		public function wc_am_api_username_field() {
			$WPL_subscription = WPL_get( 'transient', 'wpl_subscription' );
			$wpl_username = WPL_get( 'option', 'wplicense_upgrades_username' );
			$wpl_username_temp = WPL_get( 'transient', 'wpl_username' );
			$required = !empty( $WPL_subscription ) ? 'required' : false;
			if ( empty( $wpl_username ) && !empty( $wpl_username_temp ) ) {
				$username = $wpl_username_temp;
			} else {
				$username = $wpl_username;
			}
			echo "<input id='activation_email' name='" . $this->ame_data_key . "[" . $this->ame_activation_username . "]' size='50' type='text' style='min-height: 30px;' value='". $username. "' $required/>";
			if ( get_option( $this->ame_username_key ) ) {
				echo "<span class='dashicons dashicons-yes' style='color: #66ab03;vertical-align: text-top;'></span>";
			} else {
				echo "<span class='dashicons dashicons-no' style='color: #ca336c;vertical-align: text-top;'></span>";
			}
		...
		public function validate_options( $input ) {
			...
			$api_username                           = strtolower( trim( $input[ $this->ame_activation_username ] ) );

			if ( $_REQUEST[ 'option_page' ] != $this->ame_deactivate_checkbox ) {
				if ( !empty( $api_username ) ) {
					$user = $WPL_Request->check_user( 'check_domain', $api_username );
					if ( $user == $api_username ){
						update_option( $this->ame_username_key, $api_username );
					} else {
						update_option( $this->ame_username_key, null );
						WPL_set( 'transient', 'wpl_username', $api_username, false );
					}
				} else {
					update_option( $this->ame_username_key, null );
					WPL_set( 'transient', 'wpl_username', $api_username, false );
				}
				$WPL_Request->meta_items( $api_username );
				if ( $activation_status == 'Deactivated' || $activation_status == '' || $api_key == '' || $api_email == '' || $checkbox_status == 'on' || $current_api_key != $api_key ) {
					...
					if ( $activate_results[ 'activated' ] === true && ! empty( $this->ame_activated_key ) ) {
						add_settings_error( 'activate_text', 'activate_msg', sprintf( __( '%s activated. ', $this->text_domain ), esc_attr( $this->software_title ) ), 'updated' );
					...
					if ( $activate_results == false && ! empty( $this->ame_options ) && ! empty( $this->ame_activated_key ) ) {
						add_settings_error( 'api_key_check_text', 'api_key_check_error', __( 'Connection failed to the License Key API server.&nbsp;<button class="button-primary" id="wpl_request_api">Click Here</button>
						<img id="spinner_request_api" src="'. esc_attr(WPL_SPIN). '" alt="..." style="vertical-align:bottom; max-height: 30px; display:none">Then try again later.', $this->text_domain ), 'error' );
						...
						$WPL_request_api = WPL_get( 'transient', 'wpl_request_api' );
						$WPL_request_api['api'] = 'failed';
						WPL_set( 'transient', 'wpl_request_api', $WPL_request_api, false );
					}
			...
			if ( $options == 'on' && $activation_status == 'Activated' && $this->ame_options[ $this->ame_api_key ] != '' && $this->ame_options[ $this->ame_activation_email ] != '' ) {
				...
				if ( $activate_results[ 'deactivated' ] === true ) {
					...
					if ( ! empty( $this->ame_activated_key ) ) {
						...
						update_option( $this->ame_username_key, null );
						add_settings_error( 'wc_am_deactivate_text', 'deactivate_msg', __( 'API Key deactivated. ', $this->text_domain ), 'updated' );
					}
		...
		public function status( $args ) {
			...
			$args       = wp_parse_args( $defaults, $args );
			$target_url = esc_url_raw( $this->create_software_api_url( $args ) );
			$request    = wp_safe_remote_get( $target_url, array( 'timeout' => 10 ) );
